...
startInst:{[instName] x:spawnCmd,string[instName]," >& ",getenv[`scripts_dir],"logs/cmd.out &";0N! x; 		/Command to start a specific instance
			system[x];
		};
...
stopInst:{[instName] unregister[instMap[instName]];
			if[instName<>currentInst;							/only stopping if running on other instance to GW proc
				[x:stopCmd,string[instName]," >& ",getenv[`scripts_dir],"logs/cmd.out &";0N! x; 				/running awscli command to stop
				system[x]]];
		};
