...
/dynamic:0b;						/specifies whether to enable the dynamic hdb loadbalancing or just work with whatever connects
                                    /namely whether the GW process is actually a distributed HDB cluster.
...
startInst:{[instName] x:spawnCmd,string[instName],">",getenv[`scripts_dir],"logs/cmd.out 2>&1 &"; /Command to start a specific instance
			system[x];
		};
getNxtInstances:{[numInst] numInst sublist availInst}			/Give us the next instance to start
...
stopInst:{[instName] unregister[instMap[instName]];
			if[instName<>currentInst;							/only stopping if running on other instance to GW proc
				[x:stopCmd,string[instName],">",getenv[`scripts_dir],"logs/cmd.out 2>&1 &";	/running CLI command to stop
				system[x]]];
		};
